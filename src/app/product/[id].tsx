...
import { formatCurrency } from "@/utils/formaters/format-currency";
...
export default function Product() {
  ...
  return (
    ...
    <View className="flex-1">
      <Image source={product.thumbnail} className="w-full h-52" resizeMode="cover" />
      ...
      <View className="p-5 mt-8 flex-1" >
...

        {
          product.ingredients.map((ingredient) => (
            <Text key={ingredient} className="text-slate-400 font-body text-base lead ">{"\u2022"}{ingredient}</Text>
          ))
        }
      </View>
